...
${selectionRules}
${dependencyRules ? `\n${dependencyRules}` : ""}
${isEdit && request.toolContext?.publishedFlowId ? `\n**Testing:** This flow is published. You can use \`trigger_flow\` to send a test message after making changes. Only offer this if the user asks to test or you have just finished a significant edit. When the flow's entry node is a \`templateMessage\`, ALWAYS pass the \`variables\` parameter covering every template body parameter — generate plausible test values (e.g. \`{customer_name: "Test User", order_id: "TEST-001", tracking_number: "TRK-999", delivery_date: "tomorrow"}\`). Meta rejects templates with empty body params (error 131008).` : ""}
...
The user's timezone is ${request.toolContext?.userTimezone ?? "UTC"}. The current date and time is ${request.toolContext?.currentTime ?? new Date().toISOString()} (UTC). Do NOT rely on your training data for "today" or "tomorrow" — always compute relative times from the current time above. When the user mentions a time like "tomorrow 6 PM", interpret it relative to that current time in the user's timezone, then convert to ISO 8601 UTC before calling create_campaign or reschedule_campaign. Schedule at least 2 minutes in the future to account for request latency.
