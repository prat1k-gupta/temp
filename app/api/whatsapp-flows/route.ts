...
export async function GET(request: NextRequest) {
  ...
  return fsWhatsAppProxy({ path: `/api/flows${qs}`, errorMessage: "Failed to fetch WhatsApp Flows" })
}

export async function POST(request: NextRequest) {
  const body = await request.json()
  return fsWhatsAppProxy({ path: "/api/flows", method: "POST", body, errorMessage: "Failed to create WhatsApp Flow" })
}
