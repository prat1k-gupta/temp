...
export function WhatsAppQuickReplyNode({ data, selected }: { data: any; selected?: boolean }) {
  ...
  const handleUpdateChoices = (newButtons: ChoiceData[]) => {
    ...
    if (formattedChoices.length > maxButtons) {
      maybeConvertToList(formattedChoices)
    } else {
      if (data.onNodeUpdate) {
  ...
  const maybeConvertToList = (currentChoices: any[]) => {
    if (data.media) {
      setPendingConversion(() => () => doConvertToList(currentChoices))
      setShowMediaAlert(true)
    } else {
      doConvertToList(currentChoices)
    }
  }
